...
if($maintenance){
    $ackdata = array(
        "Status" => "Failed",
        "MessageString" => "Server under maintenance",
        "SubscriptionLeft" => "0"
    );
...
if($query->num_rows < 1){
    $ackdata = array(
        "Status" => "Failed",
        "MessageString" => "Username or Password is invalid!",
        "SubscriptionLeft" => "0"
    );
    PlainDie(tokenResponse($ackdata));
}
...
if($res["UID"] == NULL){
    ...
    $ackdata = array(
        "Status" => "Failed",
        "MessageString" => "your UID has been changed!",
        "SubscriptionLeft" => "0"
    );
    PlainDie(tokenResponse($ackdata));
}
